...
import io.swagger.client.models.Output
import okhttp3.*
import java.io.File
...
open class ApiClient(val baseUrl: String) {
    companion object {
        ...
        val builder: OkHttpClient.Builder = OkHttpClient.Builder().apply {
            this.callTimeout(Duration.ofSeconds(30))
    ...
    inline protected fun <reified T: Any?> responseBody(response: Response, mediaType: String = JsonMediaType): T? {
		...
		if(isJsonMime(contentType)){
            return Serializer.moshi.adapter<Output<T>>(Types.newParameterizedType(Output::class.java,T::class.java)).fromJson(response.body()?.source())?.data;
